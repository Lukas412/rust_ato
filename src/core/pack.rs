...
impl Pack {
    pub(crate) fn all_from_root<P: AsRef<Path> + ?Sized>(root: &P) -> HashMap<Namespace, Self> {
        let packs = WalkDir::new(root)
            .into_iter()
            .filter_map(Pack::namespace_and_pack_from_dir_entry);
...
impl Pack {
    fn namespace_and_pack_from_dir_entry(
        dir_entry: walkdir::Result<DirEntry>,
    ) -> Option<(Namespace, Pack)> {
...
fn is_pack_path(path: &PathBuf) -> bool {
    ...
    extensions
        .iter()
        .any(|extension| path.ends_with_str(extension))
}
